...
namespace CineZarAPI.Controllers
{
    ...
    public class PeliculaController : ControllerBase
    {
        ...
        [HttpPost("{id}/CrearOpinion")]
        public ActionResult<Pelicula> CreateOpinion(int id, int iValoracion, string strComentario, string strUsuario)
        {
            Opinion opinion = new Opinion(iValoracion, strComentario, strUsuario);
            Pelicula pelicula = peliculas.FirstOrDefault(p => p.Id == id);
            if (opinion == null)
            {
                return BadRequest("Opinion vacia");
            }
            else
            {
                pelicula.Opiniones.Add(opinion);

                pelicula.OpinionMedia = CalcularValMedia(pelicula);
        ...
        private static int CalcularValMedia(Pelicula pelicula)
        {
            int valMedia = 0;
            int valTotal = 0;
            ...
            valMedia = valTotal / pelicula.Opiniones.Count;
